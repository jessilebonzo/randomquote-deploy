...
const getRandomColor = (): string => {
  const red = Math.floor(Math.random() * 64) + 128;
  const green = Math.floor(Math.random()* 64) + 128;
  const blue = Math.floor(Math.random() * 64) + 128;
...
function App() {
  ...
  const changeQuote = () => {setQuote(getRandomQuote());
